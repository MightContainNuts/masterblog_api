    ...
    if request.method == "GET":
        ...
        if search_pattern:
            ...
            if found_posts:
                ...
                return (
                    jsonify(
                        {"Error": f"No posts found matching {search_pattern}"}
                    ),  # noqa E501
                    404,
                )

        if sort:
            sorted_posts = blog.sort_posts(
                filtered_posts=posts, sort=sort, direction=direction
            )
            posts = sorted_posts

        return (jsonify(posts), 200) if posts else []
...
def update_post(id: Index):
    ...
    if updated_post:
        return (
            jsonify(
                {f"{updated_post['id']}: {updated_post['title']}": "updated"}
            ),  # noqa E501
            200,
        )
    else:
